...
class MatrixFactory:
    ...
    def create_empty_matrix(self) -> MetricsMatrix:
        ...
        return MetricsMatrix(
            ...
            lat_matrix=np.zeros((n_services, n_samples)),
            bw_matrix=np.zeros((n_services, n_samples)),
    ...
    def save_to_file(self, matrix: MetricsMatrix, timestamp: str) -> str:
        ...
        np.save(lat_path, matrix.lat_matrix)
        np.save(bw_path, matrix.bw_matrix)
    ...
    def load_from_file(self, timestamp: str) -> MetricsMatrix:
        ...
        lat_matrix = np.load(lat_path)
        bw_matrix = np.load(bw_path)
